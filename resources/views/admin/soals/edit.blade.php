...
@section('js')
    ...
    <script src="{{ asset('template/assets/js/plugins/ckeditor5-classic/build/ckeditor.js') }}"></script>
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const kategori = '{{ $kategori }}'; // Current category from server
            ...
            const poinA = {{ $soal->poin_a }};
            const poinB = {{ $soal->poin_b }};
            const poinC = {{ $soal->poin_c }};
            const poinD = {{ $soal->poin_d }};
            const poinE = {{ $soal->poin_e }};
            ...
            const updatePoinOptions = () => {
                ...
                poinSelects.forEach(select => {
                    ...
                    let availableOptions = poinOptions[kategori].filter(opt =>
                        !selectedValues.includes(String(opt)) || String(opt) === currentValue
                    );
            ...
            document.getElementById('poin_a').value = poinA;
            document.getElementById('poin_b').value = poinB;
            document.getElementById('poin_c').value = poinC;
            document.getElementById('poin_d').value = poinD;
            document.getElementById('poin_e').value = poinE;
        });
...
@section('content-isi')
    <div class="block block-rounded">
        <div class="block-content block-content-full">
            ...
            <form action="{{ route('admin.soal.update.cat', ['id' => $soal->id]) }}" method="POST"
                    ...
                    <div class="col-lg-12 col-xl-12 overflow-hidden">
                        ...
                        <div class="col-lg-12 col-xl-12 overflow-hidden">
                            <div class="row">
                                ...
                                <div class="mb-4 col-md-6">
                                    <label class="form-label">Sub Kategori<code>*</code></label>
                            ...
                            <div class="mb-4">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <textarea class="form-control" name="option_{{ $option }}" id="option_{{ $option }}">{{ $soal->{"option_{$option}"} }}</textarea>
                                    </div>
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-2 poin-input" data-name="poin_{{ $option }}">
                                        ...
                                        @if ($option == 'a')
                                            <select class="form-control poin-select" name="poin_a" id="poin_a">
                                                @foreach ($poinOptions[$kategori] as $poin)
                                                    <option value="{{ $poin }}"
                                                        {{ $poin == $soal->poin_a ? 'selected' : '' }}>{{ $poin }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        @elseif ($option == 'b')
                                            <select class="form-control poin-select" name="poin_b" id="poin_b">
                                                @foreach ($poinOptions[$kategori] as $poin)
                                                    <option value="{{ $poin }}"
                                                        {{ $poin == $soal->poin_b ? 'selected' : '' }}>{{ $poin }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        @elseif ($option == 'c')
                                            <select class="form-control poin-select" name="poin_c" id="poin_c">
                                                @foreach ($poinOptions[$kategori] as $poin)
                                                    <option value="{{ $poin }}"
                                                        {{ $poin == $soal->poin_c ? 'selected' : '' }}>{{ $poin }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        @elseif ($option == 'd')
                                            <select class="form-control poin-select" name="poin_d" id="poin_d">
                                                @foreach ($poinOptions[$kategori] as $poin)
                                                    <option value="{{ $poin }}"
                                                        {{ $poin == $soal->poin_d ? 'selected' : '' }}>{{ $poin }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        @elseif ($option == 'e')
                                            <select class="form-control poin-select" name="poin_e" id="poin_e">
                                                @foreach ($poinOptions[$kategori] as $poin)
                                                    <option value="{{ $poin }}"
                                                        {{ $poin == $soal->poin_e ? 'selected' : '' }}>{{ $poin }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        @endif
                        ...
                        <div class="mb-4">
                            ...
                            <select name="correct_answer" id="correct_answer" class="form-control" required>
                                ...
                                <option value="e" {{ $soal->correct_option == 'e' ? 'selected' : '' }}>E</option>
                            </select>
                        </div>
